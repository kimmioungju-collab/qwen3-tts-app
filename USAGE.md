...
**레퍼런스 대사(reference_text)도 함께 저장**되므로, 다음에 불러올 때 오디오+텍스트 모두 자동 채움.

### 저장

1. Voice Clone 탭에서 레퍼런스 오디오 + 텍스트 설정
2. `💾 현재 목소리 저장` 버튼 클릭
3. 영문 ID 입력 (예: `eunwoo`) — CLI에서 `--voice eunwoo`로 사용
4. 표시 이름 입력 (예: `김은우 (큰아들)`)
5. 레퍼런스 오디오 + 레퍼런스 텍스트가 `voices/{이름}/` 에 함께 저장됨

### 로드

1. Voice Clone 탭 상단 `💾 저장된 목소리` 콤보박스에서 선택
2. `📂 불러오기` 클릭
3. 레퍼런스 오디오 + 텍스트 자동 채움 → 바로 합성 가능
...
voices/
├── eunwoo/
│   ├── config.json       # 메타데이터 (name, display_name, reference_text, language)
│   └── reference.wav     # 음성 샘플 (3초+)
├── yeonwoo/
│   ├── config.json
│   └── reference.wav
└── ...
...
> **참고**: 봇(클라비스/제니스)은 동일한 `voices/` 디렉토리를 CLI로 접근.
> GUI에서 저장한 프로필을 CLI에서도 바로 사용 가능: `python3 cli.py --voice eunwoo "텍스트"`
